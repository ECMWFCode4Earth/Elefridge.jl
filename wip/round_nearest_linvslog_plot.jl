...
ion()
pygui(true)
fig,(ax1,ax2,ax3) = subplots(1,3,figsize=(10,10),sharey=true)
...
ax1.set_ylim(-5,nvars)

ax1.set_xlim(1e-13,1e-1)
ax2.set_xlim(1e-22,1e1)
ax3.set_xlim(1e-8,1e1)

alfa=0.3
for i in 1:nvars
    if meanerror[1,i] >= meanerror[3,i]
...
ax3.fill_between(ax3.get_xlim(),[-10,-10],[-9,-9],color="C2",alpha=alfa,label="Log better than LinQuantization\nat 16 bit")
ax1.legend(loc=4,ncol=2,fontsize=8,scatterpoints=3)
ax3.legend(loc=3,fontsize=9)
