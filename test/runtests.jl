...
@testset "Bitpattern entropy" begin
    for N in [100,1000,10000,100000]
        ...
        @test isapprox(log2(N),bitentropy(rand(Float32,N)),atol=1e-1)
        @test isapprox(log2(N),bitentropy(rand(Float64,N)),atol=1e-1)
    end

    N = 1000_000   # more bitpattern than there are in 8 or 16-bit
    @test isapprox(16.0,bitentropy(rand(UInt16,N)),atol=1e-1)
    @test isapprox(16.0,bitentropy(rand(Int16,N)),atol=1e-1)

    @test isapprox(8.0,bitentropy(rand(UInt8,N)),atol=1e-1)
    @test isapprox(8.0,bitentropy(rand(Int8,N)),atol=1e-1)
end
